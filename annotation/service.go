...
// Service provides methods to interact with Annotation-related API endpoints.
...
func (api *Service) config() *signplusconfig.Config {
	return api.manager.GetAnnotation()
}
...
func (api *Service) AddEnvelopeAnnotation(ctx context.Context, envelopeID string, addEnvelopeAnnotationRequest AddEnvelopeAnnotationRequest, params AddEnvelopeAnnotationRequestParams, opts ...signplusconfig.RequestOption) ([]byte, error) {
	config := *api.config()
	...
	httpRequest := httptransport.NewRequestBuilder().WithContext(ctx).
		...
		WithConfig(config).
		WithBody(addEnvelopeAnnotationRequest).
