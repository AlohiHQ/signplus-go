...
// Service provides methods to interact with Certificate-related API endpoints.
...
func (api *Service) config() *signplusconfig.Config {
	return api.manager.GetCertificate()
}
...
func (api *Service) DownloadEnvelopeCertificate(ctx context.Context, envelopeID string, params DownloadEnvelopeCertificateRequestParams, opts ...signplusconfig.RequestOption) ([]byte, error) {
	config := *api.config()
