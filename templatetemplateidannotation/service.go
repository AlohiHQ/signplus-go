...
// Service provides methods to interact with TemplateTemplateIDAnnotation-related API endpoints.
...
func (api *Service) config() *signplusconfig.Config {
	return api.manager.GetTemplateTemplateIDAnnotation()
}
...
func (api *Service) AddTemplateAnnotation(ctx context.Context, templateID string, addTemplateAnnotationRequest AddTemplateAnnotationRequest, params AddTemplateAnnotationRequestParams, opts ...signplusconfig.RequestOption) ([]byte, error) {
	config := *api.config()
	...
	httpRequest := httptransport.NewRequestBuilder().WithContext(ctx).
		...
		WithConfig(config).
		WithBody(addTemplateAnnotationRequest).
