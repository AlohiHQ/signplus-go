...
// Service provides methods to interact with TemplateTemplateIDSetNotification-related API endpoints.
...
func (api *Service) config() *signplusconfig.Config {
	return api.manager.GetTemplateTemplateIDSetNotification()
}
...
func (api *Service) SetTemplateNotification(ctx context.Context, templateID string, setTemplateNotificationRequest SetTemplateNotificationRequest, params SetTemplateNotificationRequestParams, opts ...signplusconfig.RequestOption) ([]byte, error) {
	config := *api.config()
	...
	httpRequest := httptransport.NewRequestBuilder().WithContext(ctx).
		...
		WithConfig(config).
		WithBody(setTemplateNotificationRequest).
