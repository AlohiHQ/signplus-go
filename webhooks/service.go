...
// Service provides methods to interact with Webhooks-related API endpoints.
...
func (api *Service) config() *signplusconfig.Config {
	return api.manager.GetWebhooks()
}
...
// List webhooks
func (api *Service) ListWebhooks(ctx context.Context, listWebhooksRequest ListWebhooksRequest, params ListWebhooksRequestParams, opts ...signplusconfig.RequestOption) ([]byte, error) {
	config := *api.config()
