...
// Service provides methods to interact with Template-related API endpoints.
...
func (api *Service) config() *signplusconfig.Config {
	return api.manager.GetTemplate()
}
...
// Create new template
func (api *Service) CreateTemplate(ctx context.Context, createTemplateRequest CreateTemplateRequest, params CreateTemplateRequestParams, opts ...signplusconfig.RequestOption) ([]byte, error) {
	config := *api.config()
