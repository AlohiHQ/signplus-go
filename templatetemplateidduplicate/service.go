...
// Service provides methods to interact with TemplateTemplateIDDuplicate-related API endpoints.
...
func (api *Service) config() *signplusconfig.Config {
	return api.manager.GetTemplateTemplateIDDuplicate()
}
...
// Duplicate template
func (api *Service) DuplicateTemplate(ctx context.Context, templateID string, params DuplicateTemplateRequestParams, opts ...signplusconfig.RequestOption) ([]byte, error) {
	config := *api.config()
