...
// Service provides methods to interact with DocumentID-related API endpoints.
...
func (api *Service) config() *signplusconfig.Config {
	return api.manager.GetDocumentID()
}
...
func (api *Service) GetEnvelopeDocument(ctx context.Context, envelopeID string, documentID string, params GetEnvelopeDocumentRequestParams, opts ...signplusconfig.RequestOption) ([]byte, error) {
	config := *api.config()
