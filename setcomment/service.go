...
// Service provides methods to interact with SetComment-related API endpoints.
...
func (api *Service) config() *signplusconfig.Config {
	return api.manager.GetSetComment()
}
...
func (api *Service) SetEnvelopeComment(ctx context.Context, envelopeID string, setEnvelopeCommentRequest SetEnvelopeCommentRequest, params SetEnvelopeCommentRequestParams, opts ...signplusconfig.RequestOption) ([]byte, error) {
	config := *api.config()
