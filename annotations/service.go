...
// Service provides methods to interact with Annotations-related API endpoints.
...
func (api *Service) config() *signplusconfig.Config {
	return api.manager.GetAnnotations()
}
...
func (api *Service) GetEnvelopeAnnotations(ctx context.Context, envelopeID string, params GetEnvelopeAnnotationsRequestParams, opts ...signplusconfig.RequestOption) ([]byte, error) {
	config := *api.config()
