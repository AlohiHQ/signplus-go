...
// Service provides methods to interact with AnnotationID-related API endpoints.
...
func (api *Service) config() *signplusconfig.Config {
	return api.manager.GetAnnotationID()
}
...
func (api *Service) DeleteEnvelopeAnnotation(ctx context.Context, envelopeID string, annotationID string, opts ...signplusconfig.RequestOption) ([]byte, error) {
	config := *api.config()
