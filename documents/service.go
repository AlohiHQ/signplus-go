...
// Service provides methods to interact with Documents-related API endpoints.
...
func (api *Service) config() *signplusconfig.Config {
	return api.manager.GetDocuments()
}
...
func (api *Service) GetEnvelopeDocuments(ctx context.Context, envelopeID string, params GetEnvelopeDocumentsRequestParams, opts ...signplusconfig.RequestOption) ([]byte, error) {
	config := *api.config()
