...
// Service provides methods to interact with SetNotification-related API endpoints.
...
func (api *Service) config() *signplusconfig.Config {
	return api.manager.GetSetNotification()
}
...
func (api *Service) SetEnvelopeNotification(ctx context.Context, envelopeID string, setEnvelopeNotificationRequest SetEnvelopeNotificationRequest, params SetEnvelopeNotificationRequestParams, opts ...signplusconfig.RequestOption) ([]byte, error) {
	config := *api.config()
	...
	httpRequest := httptransport.NewRequestBuilder().WithContext(ctx).
		...
		WithConfig(config).
		WithBody(setEnvelopeNotificationRequest).
