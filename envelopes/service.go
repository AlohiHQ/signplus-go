...
// Service provides methods to interact with Envelopes-related API endpoints.
...
func (api *Service) config() *signplusconfig.Config {
	return api.manager.GetEnvelopes()
}
...
// List envelopes
func (api *Service) ListEnvelopes(ctx context.Context, listEnvelopesRequest ListEnvelopesRequest, params ListEnvelopesRequestParams, opts ...signplusconfig.RequestOption) ([]byte, error) {
	config := *api.config()
