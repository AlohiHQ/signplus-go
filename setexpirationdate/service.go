...
// Service provides methods to interact with SetExpirationDate-related API endpoints.
...
func (api *Service) config() *signplusconfig.Config {
	return api.manager.GetSetExpirationDate()
}
...
func (api *Service) SetEnvelopeExpirationDate(ctx context.Context, envelopeID string, setEnvelopeExpirationDateRequest SetEnvelopeExpirationDateRequest, params SetEnvelopeExpirationDateRequestParams, opts ...signplusconfig.RequestOption) ([]byte, error) {
	config := *api.config()
	...
	httpRequest := httptransport.NewRequestBuilder().WithContext(ctx).
		...
		WithConfig(config).
		WithBody(setEnvelopeExpirationDateRequest).
