...
// Service provides methods to interact with SigningSteps-related API endpoints.
...
func (api *Service) config() *signplusconfig.Config {
	return api.manager.GetSigningSteps()
}
...
func (api *Service) AddEnvelopeSigningSteps(ctx context.Context, envelopeID string, addEnvelopeSigningStepsRequest AddEnvelopeSigningStepsRequest, params AddEnvelopeSigningStepsRequestParams, opts ...signplusconfig.RequestOption) ([]byte, error) {
	config := *api.config()
